                               ...
                               void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        if (strncmp(event->data, "ADD", 3) == 0)
        {
            ...
            // Topic
            snprintf(ack_topic, sizeof(ack_topic), "esp32/ack/%s", DEVICE_ID);
        ...
        else if (strncmp(event->data, "RM", 2) == 0)
        {
            ...
            snprintf(
                ack_msg,
                sizeof(ack_msg),
                "{\"device_id\":\"%s\",\"RM\":\"%.*s\",\"status\":\"received\",\"gmail\":\"%s\"}",
                DEVICE_ID,
                event->data_len,
                event->data,
                cached_gmail);
...
void app_main(void)
{
    ...
    // Build topic
    snprintf(will_topic, sizeof(will_topic), "esp32/status/%s", DEVICE_ID);
